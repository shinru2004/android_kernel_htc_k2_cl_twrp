...
static void k2cl_amp_speaker(bool en)
{
	gpio_set_value(MSM_AUD_SPK_EN, en);
}

static void k2cl_amp_receiver(bool en)
{
}

static void k2cl_amp_headset(bool en)
{
}

static void k2cl_amp_hac(bool en)
{
}

struct htc_8930_gpio_pdata htc_audio_gpio = {
	.amp_speaker = k2cl_amp_speaker,
	.amp_receiver = k2cl_amp_receiver,
	.amp_headset = k2cl_amp_headset,
	.amp_hac = k2cl_amp_hac,
	.mi2s_gpio = {
	{
	...
	.i2s_gpio = {
	{
		.gpio_no = MSM_V_BOOST_5V_EN,
		.gpio_name = "AUD_FM_I2S_CLK",
	},
	{
		.gpio_no = MSM_V_WL_IO_1V8_EN,
		.gpio_name = "AUD_FM_I2S_WS",
	},
	{
		.gpio_no = MSM_NC_GPIO_57,
...
static int k2cl_get_hw_revision(void)
{
	int audio_hw_rev;
...
static int k2cl_get_hw_component(void)
{
	return 0;
}

static int k2cl_enable_digital_mic(void)
{
	return 0;
}

static void k2cl_set_q6_effect_mode(int mode)
{
	pr_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

static int k2cl_get_q6_effect_mode(void)
{
...
static int k2cl_get_24b_audio(void)
{
	return 1;
}
...
static struct acoustic_ops acoustic = {
	...
	.get_hw_component = k2cl_get_hw_component,
	.enable_digital_mic = k2cl_enable_digital_mic,
	.get_24b_audio = k2cl_get_24b_audio,
};
...
module_init(k2cl_audio_init);

static void __exit k2cl_audio_exit(void)
{
	pr_info("%s", __func__);
}
module_exit(k2cl_audio_exit);

MODULE_DESCRIPTION("ALSA Board K2CL");

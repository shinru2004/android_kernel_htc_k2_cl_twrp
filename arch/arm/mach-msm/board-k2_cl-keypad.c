...
#define MODULE_PARAM_PREFIX "board_k2_cl."
...
static struct gpio_event_direct_entry k2_cl_keypad_input_map[] = {
	{
		.gpio = MSM_PWR_KEYz,
		.code = KEY_POWER,
	},
...
static void k2_cl_setup_input_gpio(void)
{
...
static struct gpio_event_platform_data k2_cl_keypad_data = {
	...
	.info_count = ARRAY_SIZE(k2_cl_keypad_info),
};

static struct platform_device k2_cl_keypad_input_device = {
	.name = GPIO_EVENT_DEV_NAME,
	.id = 0,
	.dev		= {
		.platform_data	= &k2_cl_keypad_data,
	},
};
static struct keyreset_platform_data k2_cl_reset_keys_pdata = {
...
struct platform_device k2_cl_reset_keys_device = {
	.name = KEYRESET_NAME,
	.dev.platform_data = &k2_cl_reset_keys_pdata,
};

int __init k2_cl_init_keypad(void)
{
	printk(KERN_DEBUG "[KEY]%s\n", __func__);

	if (platform_device_register(&k2_cl_reset_keys_device))
		printk(KERN_WARNING "%s: register reset key fail\n", __func__);

	return platform_device_register(&k2_cl_keypad_input_device);
}

...
static void send_usb_connect_notify(struct work_struct *w)
{
	...
	if (board_mfg_mode() == 5 && motg->connect_type == CONNECT_TYPE_USB) { 
...
#define USB_PHY_1P975_VOL_MIN	1975000 
#define USB_PHY_1P975_VOL_MAX	1975000 
...
static int msm_hsusb_ldo_init(struct msm_otg *motg, int init)
{
	...
	if (init) {
		...
		dev_info(&pdev->dev, "system_rev= %d, engineerid= %d, skuid= %d\n", system_rev, engineerid, skuid);
		if(system_rev == XC && skuid == 0x03){
			rc = regulator_set_voltage(hsusb_1p8, USB_PHY_1P975_VOL_MIN,
				USB_PHY_1P975_VOL_MAX);
			dev_info(&pdev->dev, "LDO4 set to 1.975 v\n");
		}
		else if(system_rev == XC && engineerid == 1){
			rc = regulator_set_voltage(hsusb_1p8, USB_PHY_1P975_VOL_MIN,
				USB_PHY_1P975_VOL_MAX);
			dev_info(&pdev->dev, "LDO4 set to 1.975 v\n");
		}
			...
			rc = regulator_set_voltage(hsusb_1p8, USB_PHY_1P8_VOL_MIN,
				USB_PHY_1P8_VOL_MAX);
			dev_info(&pdev->dev, "LDO4 set to 1.8 v\n");
		}
#else
		rc = regulator_set_voltage(hsusb_1p8, USB_PHY_1P8_VOL_MIN,
			USB_PHY_1P8_VOL_MAX);
